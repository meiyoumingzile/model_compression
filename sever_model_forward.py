...
def whileForward(path):
    while(True):
        realTime, mint, maxt = getMnnKernelTime(path)
        pic_1 = pic.tobytes()
        print(("avgTime:" + str(realTime)).encode('utf-8'))
        time.sleep(1)
...
def receive_file(file_path):
    ...
    while(True):
        ...
        while (True):
            ...
            if dataStr.find("start run")!=-1 or isStart:
                # file_name=data[indi+len(b'EOF'):].decode('utf-8')
                # print(dataStr,file_name)
                thread = threading.Thread(target=whileForward, args=(file_path + file_name,))
                thread.start()

                # client_socket.sendall(pic_1 + b'EOF')
                # client_socket.sendall(("avgTime:" + str(realTime)).encode('utf-8')+ b'EOF')
            elif dataStr.find("end run")!=-1:
                server_socket.close()
                break
        break
